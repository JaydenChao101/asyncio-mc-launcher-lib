...
def linkcode_resolve(domain: str, info: dict[str, str]):
    ...
    module_name = info["module"].removeprefix("launcher_core")

    obj = getattr(getattr(launcher_core, module_name), info["fullname"])
...
def write_module_file(in_path: pathlib.Path, out_dir: pathlib.Path) -> None:
    with open(os.path.join(out_dir, in_path.stem + ".rst"), "w", encoding="utf-8") as f:
        ...
        f.write("==========================\n")
        f.write(f".. automodule:: launcher_core{in_path.stem}\n")
...
def write_modules() -> None:
    MODULE_ORDER = ("command", "install", "natives", "microsoft_account", "utils", "news", "java_utils", "forge", "fabric", "quilt", "runtime", "vanilla_launcher", "mrpack", "exceptions", "_types", "microsoft_types")
